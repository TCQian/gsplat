...
torch::Tensor get_tile_bin_edges_tensor(
    int num_intersects, const torch::Tensor &isect_ids_sorted, const std::tuple<int, int, int>& tile_bounds
) {
    CHECK_INPUT(isect_ids_sorted);
    int tile_x = std::get<0>(tile_bounds);
    int tile_y = std::get<1>(tile_bounds);
    int tile_z = std::get<2>(tile_bounds);
    int num_tiles = tile_x * tile_y * tile_z;
